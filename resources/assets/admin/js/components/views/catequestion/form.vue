	...
	<div class="content-section" style="padding-bottom:120px">
	    ...
	    <form @submit.prevent="save()" @keydown="errors.clear($event.target.name)">
	        <div class="form-group">
	            <label>Name</label>
	            <input  v-model="form.name" name="name" class="form-control" placeholder="Name Cate Question" />
	        ...
	        <div v-if="create" class="form-group">
	            <label>Slug</label>
	            <input  v-model="form.slug" name="slug" class="form-control" placeholder="Slug Cate Question" />

                <p class="alert alert-danger" v-if="errors.has('slug')" v-text="errors.get('slug')"></p>
	        </div>
            <div v-else class="form-group">
                <label>Slug</label>
                <input disabled  v-model="form.slug" name="slug" class="form-control" placeholder="Slug Cate Question" />
            </div>
	        <div class="form-group">
      ...
      export default {
        ...
        data () {
          return {
            ...
            link: '/api/catequestion',
            redirect: 'AdminCateQuestionIndex',
            method: 'post',
            chose: false,
            display: 'Show',
            errors : new Error
        ...
        beforeMount() {
        	if(this.$route.meta.mode == 'edit') {
        		this.title = 'Edit Cate Question',
        		this.link = '/api/catequestion/'+this.$route.params.id,
        		this.method = 'put',
                this.create = false,
                this.fetchData()
        	}
        ...
        methods: {
            fetchData() {
                ...
                }).catch(error => vm.$router.push({ name: 'AdminCateQuestionIndex' }))
            },
            save() {
                var vm = this
                axios({
                    ...
                    if(response.data.saved) {
                        ...
                        vm.$swal("Success!", "Save Cate Question!", "success", {
                          button: "OK!",
                        });
                        vm.$router.push({ name: 'AdminCateQuestionIndex' })
                    }
