<template>
  <div id="root">
    ...
    <button @click="$router.push({ name: 'AdminPostsCreate' })" class="btn btn-info col-xs-2 pull-right">New</button>
        ...
        <tbody v-for="post in posts.data">
            <tr class="table-content" align="center">
                ...
                <td class="is-delete center"><ConfirmModal
                    ...
                    ></ConfirmModal></td>
                <td class="is-edit center"> <router-link :to="{ name: 'AdminPostsEdit', params:{id: post.id } }"><span class="btn btn-xs btn-info"><span class='glyphicon glyphicon-pencil'></span><span> Edit</span></span></router-link></td>
            </tr>
        </tbody>
    </table>
...
import ConfirmModal from '../blocks/confirm.vue'
      ...
      export default {
        ...
        methods: {
            ...
            updateSearch() {
                if(this.search!='') {
                    this.link = '/api/posts/search/'+ this.search
                } else {
                    this.link = '/api/posts'
                }
                this.fetchData();
            ...
            goTo() {
                ...
                if(this.posts.current_page <= this.posts.last_page) {
                    var prefix;
                    if(this.search != '') {
                        prefix = '/api/posts/search/'+this.search+'?page='
                    } else {
                        prefix = '/api/posts?page='
                    }
