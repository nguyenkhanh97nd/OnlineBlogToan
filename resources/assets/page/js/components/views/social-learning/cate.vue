		...
		<div class="row">
			<div class="col-md-3">
				...
				<ul class="ul-catequestion">
					<li v-for="cate in catequestion"><span class="glyphicon glyphicon-book" :style="'color:' + cate.color "></span> <router-link  :to="{ name: 'ClientSocialLearningCateQuestion', params: { slugCateQuestion: cate.slug } }"><a  @click="changeCate(cate.slug)" class="link-learn">{{ cate.name }}</a></router-link>

							
							<ul v-if="cate_param == cate.slug" style="display: block" class="ul-subcatequestion">
								<li v-for="subcate in cate.sub_cate_question"><router-link  :to="{ name: 'ClientSocialLearningSubCateQuestion', params: { slugCateQuestion: cate.slug, slugSubCateQuestion: subcate.slug } }"><a>{{ subcate.name }}</a></router-link></li>
							</ul>
			...
			<div class="showQuestionForm" v-if="current_user">
				...
				<div class="media-body body-feed-item">
					...
					<div class="sort-left div_select_subcate" layout="row" style="margin-left:5px; display: none;" layout-align="space-between center">
				    		...
				    		<select v-model="point_set" required placeholder="Số điểm"  class="md-no-underline custom-md-select-cate">
				    				<option value="" disabled>Số điểm</option>
	...
	export default {
		...
		methods: {
			...
			fetchData() {
				var vm = this
				vm.getCateQuestion()
				vm.getFeeds(vm.cate_param)
			},
			getFeeds(slug) {
				var vm = this
				var link = vm.link_cate_question + "/" + slug
